...
static uint64_t countqbytes(uint8_t *blkbuff, uint64_t bases)
{
    uint64_t rbases = 0;
    uint64_t nbytes = 0;
    while (rbases < bases) {
                       ...
                       uint8_t  len)
{
    ...
    while(nbases--) {
        ...
        decoded[nbases] = seq_dec_tableSQZ[byte]; //Write corresponding base
...
static uint64_t pdecode(uint8_t *blkbuff,
                        uint64_t seqoffset,
                        uint64_t buffsize,
                        uint8_t *buff,
                        uint8_t fqflag)
                            ...
                            uint8_t  fqflag)
{
    ...
    uint64_t qdecoded  = 0;
    uint8_t  spacer = 0;
    //TODO check this logic for partial decoding
    //Compute how much sequence can be decoded
    todecode = seqlen < prevbytes ? 0 : seqlen - prevbytes;
    todecode = todecode < buffsize ? todecode : buffsize;
    qdecoded = seqlen < prevbytes ? prevbytes - seqlen - 3 : 0;
    qdecoded = qdecoded < buffsize ? qdecoded : buffsize;
    //fprintf(stderr, ">>%lu seqlen\n", seqlen);
    //fprintf(stderr, ">>%lu buffsize\n", buffsize);
    //fprintf(stderr, ">>%lu bases need decoding buffsize: %lu\n",
    //        todecode, buffsize);
...
*/
static uint64_t sqz_codeblksize(uint8_t *blkbuff, uint8_t fqflag)
{
    uint64_t bases   = 0;
    uint64_t blkpos  = 0;
    uint64_t blkpos2 = 0;
                              ...
                              uint64_t      blklen)
{
    ...
    //if (blkleft) {
    sqz_bit2decode(mer + blkidx, outbuff + outpos, blklen % 32);
    codepos += B64;
    outpos += blklen % 32;
    //}
    *wbytes += outpos;
                               ...
                               uint64_t length)
{
    ...
    while (decoded != length) {
        ...
        decoded += ++count; //Count[0-31], real length [1-32]
        byte++;
    }
    return byte;
}
    ...
    while (length > 0) {
        ...
        if (length) {     //Partial sequence decoding
            ...
            if (nflag) {
                ...
                codepos += nbytes;
                sqz_writens(nnum, outbuff + seqpos);
                seqpos += nnum;
                length -= nnum;
            }
            else {
                         ...
                         char fqflag)       //0 - fasta, 1 - fastq
{
    ...
    */
    ...
    if (prevbytes) {
        ...
        buff[wbytes++] = NL;
        prevbytes = 0;
        //Update buffer size to next sequence
